...
@getUserEmail.route('/user/<string:email>', methods=['GET','PUT','DELETE'])
def getUsersBy_id(email):
        ...
        if user:
            try:
                ...
                if not all(field.strip() for field in (_firstname, _lastname, _email, _username, _password, _dob)):
                    ...
                    # Update user details
                    cursor.execute("UPDATE users SET firstname = %s, lastname = %s, email = %s, dob = %s, age = %s, username = %s, password = %s WHERE email = %s",
                                (_firstname, _lastname, _email, dob, age, _username, hashed_password, email))
                    mysql.connection.commit()
                    return jsonify({"message": f"User with email({email}) details updated successfully."}), 200
